...
export async function POST(request: Request) {
  ...
  if (!body?.messageId || !body?.attachmentId || !body?.mimeType || !body?.filename) {
    return NextResponse.json(
      { error: 'messageId, attachmentId, mimeType, filename are required' },
      { status: 400 }
    );
  }

  const n8nUrl = `${process.env.N8N_BASE_URL?.replace(/\/$/, '')}/webhook/gmail-extract`;
